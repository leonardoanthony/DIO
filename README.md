# Formação Flutter Specialist - DIO
 
- [X] Introdução ao Flutter 
   - [X] Introdução ao Flutter
   - [X] Ambiente de Desenvolvimento Flutter
   - [X] Criando meu primeiro projeto em Flutter
   - [X] Subindo o seu Projeto Flutter no Github
- [ ] Linguagem de Programação Dart 
   - [X] Fundamentos da linguagem Dart
   - [X] Estruturas Condicionais e De Repetição em Dart
   - [X] Dominando Funções em Dart
- [ ] Navegação e Widgets em Flutter
- [ ] Persistência de Dados com SQLite
- [ ] Consumindo APIs com Flutter
- [ ] Trabalhando com Pacotes Externos em Flutter
- [ ] Gerenciamento de Estadoe e Padrões em Flutter
- [ ] Introdução ao Firebase
- [ ] Publicando os Seus Apps Flutter

 

